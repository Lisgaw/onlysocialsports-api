...
// ─── Context helpers (time of day, season, experience level) ──────────────────
...
function generateBotBio({ locale, sportName, cityName, persona, botId, totalMatches = 0 }) {
  ...
  const expLevel = getExperienceLevel(totalMatches);
  const base = pool[hashSeed(`${s}-${c}-${resolvedPersona || ''}-${expLevel}`) % pool.length];
  if (persona) return `${base} Style: ${persona}.`;
  return base;
}

// ─── Listing description ───────────────────────────────────────────────────────
function generateListingDesc({ name, sport, locale, city, botId, totalMatches = 0 }) {
  const s = sport || 'sport';
  const templates = {
    tr: [
  ...
  const expLevel = getExperienceLevel(totalMatches);
  return pool[hashSeed(`${name || ''}-${s}-${city || ''}-${persona}-${season}-${expLevel}`) % pool.length];
}
...
function generateBotSocialPost({ locale, sportName, cityName, botName, botId, totalMatches = 0 }) {
  const s = sportName || 'sport';
  const c = cityName || '';
  const n = botName || '';
  const templates = {
  ...
  const season = getSeason();
  const expLevel = getExperienceLevel(totalMatches);
  return pool[hashSeed(`${n}-${s}-${c}-post-w${weekNum}-${persona}-${timeBucket}-${season}-${expLevel}`) % pool.length];
}

// ─── Bot social topic listing (ready-made social templates) ──────────────────
function generateBotSocialTopicListing({ locale, cityName, botName, botId, totalMatches = 0 }) {
  const c = cityName || '';
  const n = botName || '';
  const templates = {
    tr: [
  ...
  const season = getSeason();
  const expLevel = getExperienceLevel(totalMatches);
  const idx = hashSeed(`${n}-${c}-${locale}-w${weekBucket}-topic-${persona}-${timeBucket}-${season}-${expLevel}`) % pool.length;
...
function generateBotSocialListing({ locale, sportName, cityName, botName, botId, totalMatches = 0 }) {
  // Mix sports-oriented social posts with ready-made social topic templates.
  if (Math.random() < 0.45) {
    const topic = generateBotSocialTopicListing({ locale, cityName, botName, botId, totalMatches });
    return { kind: 'TOPIC', title: topic.title, content: topic.content };
  }
  return {
    kind: 'SPORT',
    title: null,
    content: generateBotSocialPost({ locale, sportName, cityName, botName, botId, totalMatches }),
  };
}
...
function generateBotComment({ locale, sportName, posterName, botId, postId, attempt = 0, totalMatches = 0 }) {
  ...
  const timeBucket = getTimeBucket();
  const expLevel = getExperienceLevel(totalMatches);
  const seedBase = `${language}-${s || 'generic'}-${botId || ''}-${postId || ''}-${weekBucket}-${attempt}-${persona}-${timeBucket}-${expLevel}`;
