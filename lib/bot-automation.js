...
function appendCommentEmojiFlavor(baseText, seed) {
  ...
  const emojiPool = ['❤️', '🔥', '👏', '🙌', '✨', '💪', '🤝', '😊'];
  const first = emojiPool[hashSeed(`${seed}-emoji-1`) % emojiPool.length];
  const addSecond = (hashSeed(`${seed}-emoji-2`) % 100) < 20;

  let suffix = ` ${first}`;
  if (addSecond) {
    const second = emojiPool[(hashSeed(`${seed}-emoji-3`) + 3) % emojiPool.length];
    if (second !== first) suffix += second;
  }
