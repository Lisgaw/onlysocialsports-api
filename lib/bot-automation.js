...
function generateBotSocialPost({ locale, sportName, cityName, botName }) {
  ...
  return pool[hashSeed(`${n}-${s}-${c}-post-w${weekNum}`) % pool.length];
}
...
function generateBotSocialTopicListing({ locale, cityName, botName }) {
  ...
  const idx = hashSeed(`${n}-${c}-${locale}-w${weekBucket}-topic`) % pool.length;
...
function generateBotComment({ locale, sportName, posterName, botId, postId, attempt = 0 }) {
  ...
  const pn = posterName || '';
  const weekBucket = Math.floor(Date.now() / (86400000 * 7));
  const seedBase = `${language}-${s || 'generic'}-${botId || ''}-${postId || ''}-${weekBucket}-${attempt}`;
