...
// ─── Deterministic + unique avatar URL ────────────────────────────────────────
// randomuser has only 100 portraits per gender and causes frequent collisions.
// DiceBear gives effectively unbounded unique avatars when seed is unique.
function buildBotAvatarUrl({ gender, seed }) {
  const isFemale = (gender || '').toUpperCase() === 'FEMALE';
  const style = isFemale ? 'lorelei' : 'adventurer';
  const safeSeed = encodeURIComponent(String(seed || uuid()));
  return `https://api.dicebear.com/9.x/${style}/png?seed=${safeSeed}&size=256`;
}
