...
function generateBotComment({ locale, sportName, posterName }) {
  ...
  if (!s) {
    ...
    const pool = genericTemplates[locale] || genericTemplates.en;
    return pool[hashSeed(`${pn}-${locale}-generic-comment`) % pool.length];
  }
  ...
  const pool = templates[locale] || templates.en;
  return pool[hashSeed(`${pn}-${s}-comment`) % pool.length];
}
